...
class trackingDataSerializer(serializers.ModelSerializer):
    """Full tracking serializer including nested vehicle + route objects."""
...
class trackingSerializer(serializers.ModelSerializer):
    """Flat tracking serializer for write operations."""
    class Meta:
        model = Tracking
        fields = "__all__"
...
class TrackingSerializer(serializers.ModelSerializer):
    tracking_route = RouteSerializer(read_only=True)
    # trip_ended is now a @property on the model — expose it as a read-only field
    trip_ended = serializers.BooleanField(read_only=True)
